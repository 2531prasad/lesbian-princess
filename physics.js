...
(function (root, factory) {
  ...
  const BASE_SPEED=175, RAMP=.035, GRAVITY=1100, JUMP=515, FINISH=Infinity;
  ...
  class Runner {
    ...
    jump(){if(this.status!=='playing')return;if(this.grounded){this.jumpBuffer=.14;this.launch();}else if(this.jumpsUsed<2)this.launch();}
    launch(){if(this.jumpsUsed>=2)return;this.vy=JUMP;this.grounded=false;this.support=null;this.jumpBuffer=0;this.jumpsUsed++;this.events.push({type:'jump',jumpsUsed:this.jumpsUsed});}
    ...
    step(dt){
      ...
      this.elapsed+=dt;this.speed=BASE_SPEED*Math.exp(RAMP*this.elapsed)*this.pathMultiplier;
      const travel=BASE_SPEED/RAMP*(Math.exp(RAMP*this.elapsed)-Math.exp(RAMP*previousElapsed))*this.pathMultiplier;
  ...
  return{Runner,CAT,BASE_SPEED,RAMP,FINISH,elevationAt,gapAt,isGap,sweep,properties};
});
